    ...
    if ((premiumStoryboard || getFreeCloudQualityEnabled()) && chapters.length > 0 && usable.length > 0) {
      try {
        ...
        const refined = await requestHybridText({
          ...
          // The previous 120s client abort was shorter than legitimate local-model
          // generation for long, evidence-heavy manuscripts. Keep the browser request
          // alive long enough for the broker to complete its governed provider/fallback
          // routing instead of cancelling exactly at two minutes.
