# Production Protocol v1.1 — Candidate
...
This document, its executable policy module, calibration fixtures, and freeze manifest together define **Production Protocol v1.1 — Candidate**. Once the freeze manifest is created, any substantive change to these protocol artifacts requires a new candidate identifier and a new manifest. A dry run verifies the measurement process only; it is not production validation or competitive evidence.
